...
When the worker is enabled, the public API remains `POST /api/tools/[slug]`; the Next.js route proxies internally to worker endpoints such as `POST /api/ping`. If `DIAGNOSTICS_WORKER_TOKEN` is set, the route sends it to the worker as a bearer token.
